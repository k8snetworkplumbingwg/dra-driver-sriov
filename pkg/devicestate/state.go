...
func (s *Manager) handleRDMADevice(ctx context.Context, deviceInfo resourceapi.Device, pciAddress, deviceName string) ([]*cdispec.DeviceNode, []string, error) {
	...
	// AttributeRDMACapable was already checked above and returns nil for
	// non-RDMA devices. Reaching here means the allocated device is RDMA
	// capable, so missing RDMA nodes is an error.
