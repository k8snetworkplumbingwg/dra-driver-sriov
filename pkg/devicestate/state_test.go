...
var _ = Describe("Manager", Serial, func() {
	...
	Context("applyConfigOnDevice", func() {
		...
		It("skips RDMA preparation for vfio-pci even when the device is RDMA capable", func() {
			cdiHandler, err := cdi.NewHandler(GinkgoT().TempDir())
			Expect(err).NotTo(HaveOccurred())
		...
		It("returns an error when a kernel-driver RDMA allocation finds no RDMA devices", func() {
			m := &Manager{
				allocatable: drasriovtypes.AllocatableDevices{
					"device1": {
						Name: "device1",
