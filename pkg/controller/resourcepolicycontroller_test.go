...
var _ = Describe("deviceMatchesFilters linkType", func() {
	...
	filter := func(lt string) sriovdrav1alpha1.ResourceFilter {
	...
	It("matches ethernet device with 'eth' (lowercase)", func() {
		...
		Expect(r.deviceMatchesFilter(d, filter("eth"))).To(BeTrue())
	})
	...
	It("matches infiniband device with 'ib' (lowercase)", func() {
		...
		Expect(r.deviceMatchesFilter(d, filter("ib"))).To(BeTrue())
	})
	...
	It("does not match ethernet device with 'ib' filter", func() {
		d := makeDevice(sriovconsts.LinkTypeEthernet)
		Expect(r.deviceMatchesFilter(d, filter("ib"))).To(BeFalse())
	})

	It("does not match infiniband device with 'eth' filter", func() {
		d := makeDevice(sriovconsts.LinkTypeInfiniband)
		Expect(r.deviceMatchesFilter(d, filter("eth"))).To(BeFalse())
	})
...
var _ = Describe("getPolicyDeviceMap", func() {
	...
	It("does not broaden a non-pci filter to PCI inventory in a mixed-filter config", func() {
		vendor := "8086"
		virtualPci := "0000:aa:00.1"
		...
		matched := r.getPolicyDeviceMap(policies, nil)
		Expect(matched).To(HaveLen(2))
		Expect(matched).ToNot(HaveKey("0000-aa-00-2"))
	})
})
